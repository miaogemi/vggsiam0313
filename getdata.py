...
#datafolder = otb100-train,"shuffled_data_list" , "tfrecords", num_video = 64
def prepare_shuffled_list(data_folder, output_filename, output_directory, num_video):
    ...
    data_folder = os.path.join(cur_dir, data_folder)#路径拼接
    ...
    for video_folder in video_folder_list:
        ...
        #get a list of dirs in data_folder, in each of which contains a training vedio
        file_list = [dir for dir in os.listdir(img_folder) if os.path.isfile(os.path.join(img_folder, dir))]
        ...
        z = os.path.join(img_folder, img_list[0])
        ...
        for _example in _examples[1: ]:
            ...
            x = os.path.join(img_folder, _example[0])
            x_img = cv2.imread(x)
...
if __name__ == "__main__":
    prepare_shuffled_list("OTB100-train", "shuffled_data_list", "tfrecords", num_video = 62)
